...
(function ($) {
	{ /** CSS & glyphs               **/
		...
		var insert = function (place, st, diff) {
			if (st == "@enterkey") {
				...
				if (pruneStr(elemIn.mathquill('latex')) != '') {
					...
					place.find('span#in-' + idNum).mathquill().click(function () {
						copyToCurrent(place, $(this));
					});
					place.find('span#out-' + idNum).mathquill().click(function () {
						copyToCurrent(place, $(this));
					});
			...
			if (typeof (place.find(".inputDisplay").prop) === "undefined") // .prop() doesn't exist in this version of jQuery.
				place.find(".inputDisplay").animate({
					scrollTop: place.find(".inputDisplay").attr("scrollHeight")
				}, 100);
			else
				place.find(".inputDisplay").animate({
					scrollTop: place.find(".inputDisplay").prop("scrollHeight")
				}, 100);
		}
		...
		var init = function (place, params) {
			...
			{ // Click handlers.
				$this.find('button#piBtn').click(function () {
					insert($this, '\\pi', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#expBtn').click(function () {
					insert($this, '\\e', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#clearBtn').click(function () {
				...
				$this.find('button#powBtn').click(function () {
					insert($this, '^', 0 /*'%1^', -1*/ );
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#quadBtn').click(function () {
					insert($this, '%1^2', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#decimalBtn').click(function () {
					insert($this, '.', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#enterBtn').click(function () {
					insert($this, '@enterkey', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#addBtn').click(function () {
					insert($this, '+', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#subBtn').click(function () {
					insert($this, '-', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#divBtn').click(function () {
					insert($this, '/', 0 /*'\\frac{%1}{}', -1*/ );
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#sqrtBtn').click(function () {
					insert($this, '\\sqrt{%1}', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#logBtn').click(function () {
					insert($this, '\\log\\left(%1', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#ansBtn').click(function () {
					insert($this, '\\ans\\left(%1', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#lnBtn').click(function () {
					insert($this, '\\ln\\left(%1', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#arcsinBtn').click(function () {
					insert($this, '\\sin^{-1}\\left(%1', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#arccosBtn').click(function () {
					insert($this, '\\cos^{-1}\\left(%1', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#arctanBtn').click(function () {
					insert($this, '\\tan^{-1}\\left(%1', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#sinBtn').click(function () {
					insert($this, '\\sin\\left(%1', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#cosBtn').click(function () {
					insert($this, '\\cos\\left(%1', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#tanBtn').click(function () {
					insert($this, '\\tan\\left(%1', -1);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
					}, $(this).parent().data('params')));
				});
				$this.find('button#prodBtn').click(function () {
					insert($this, '*', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#leftParenBtn').click(function () {
					insert($this, '(', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#absBtn').click(function () {
					insert($this, '|', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#factorialBtn').click(function () {
					insert($this, '!', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b0').click(function () {
					insert($this, '0', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b1').click(function () {
					insert($this, '1', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b2').click(function () {
					insert($this, '2', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b3').click(function () {
					insert($this, '3', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b4').click(function () {
					insert($this, '4', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b5').click(function () {
					insert($this, '5', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b6').click(function () {
					insert($this, '6', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b7').click(function () {
					insert($this, '7', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b8').click(function () {
					insert($this, '8', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button#b9').click(function () {
					insert($this, '9', 0);
					$this.trigger('calculator_press', $.extend({
						senderID: $this.attr('id'),
						buttonID: this.id
				...
				$this.find('button').keypress(
						function (e) {
							...
							switch (c) {
								case '0':
									$this.find('button#b0').click();
									break;
								case '1':
									$this.find('button#b1').click();
									break;
								case '2':
									$this.find('button#b2').click();
									break;
								case '3':
									$this.find('button#b3').click();
									break;
								case '4':
									$this.find('button#b4').click();
									break;
								case '5':
									$this.find('button#b5').click();
									break;
								case '6':
									$this.find('button#b6').click();
									break;
								case '7':
									$this.find('button#b7').click();
									break;
								case '8':
									$this.find('button#b8').click();
									break;
								case '9':
									$this.find('button#b9').click();
									break;
								case 'e':
									$this.find('button#expBtn').click();
									break;
								case '²':
									$this.find('button#quadBtn').click();
									break;
								case '/':
									$this.find('button#divBtn').click();
									break;
								case 'c':
									$this.find('button#clearBtn').click();
									break;
								case ',':
									$this.find('button#decimalBtn').click();
									break;
								case '*':
									$this.find('button#prodBtn').click();
									break;
								case '-':
									$this.find('button#subBtn').click();
									break;
								case '+':
									$this.find('button#addBtn').click();
									break;
								case '(':
									$this.find('button#leftParenBtn').click();
									break;
								case '|':
									$this.find('button#absBtn').click();
									break;
								case '!':
									$this.find('button#factorialBtn').click();
									break;
							}
						}
					)
			...
			'unittest': function (params) {
				tests = [{
					...
					},
					{
						...
						clause: '\\frac{8}{12}',
						check: function (ans) {
							return Math.abs(0.66666666 - ans) < 0.00001;
						}
					},
					{
						method: 'calculate',
					...
					{
						method: 'calculate',
						clause: '\\sin\\left(\\pi\\right)',
						check: function (ans) {
							return Math.abs(0 - ans) < 0.00001;
						}
					},
					{
						method: 'calculate',
						clause: '\\sin\\left(\\arccos\\left( 1 \\right)\\right)',
						check: function (ans) {
							return Math.abs(0 - ans) < 0.00001;
						}
					},
